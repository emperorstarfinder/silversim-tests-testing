...
{
    public sealed class InfoTests : ITest
    {
        ...
        public bool Run()
        {
            ...
            m_Log.Info("Fetching sessioninfo 1");
            info = m_UserSessionService[sessionID];
            m_Log.Info("Testing for value 1");
            if (!info.TryGetValue("testassoc1", "testvarname1", out e))
            {
                return false;
            }
            m_Log.Info("Testing equality of value 1");
            if(e.Value != "testvalue1" || e.ExpiryDate == null)
            {
                return false;
            }
            m_Log.Info("Testing for value 2");
            if (!info.TryGetValue("testassoc2", "testvarname2", out e))
            {
                return false;
            }
            m_Log.Info("Testing equality of value 2");
            if (e.Value != "testvalue2" || e.ExpiryDate != null)
            {
                return false;
            }

            m_Log.Info("Fetching sessioninfo 2");
            if (!m_UserSessionService.TryGetValue(sessionID, out info))
            {
                return false;
            }
            m_Log.Info("Testing for value 1");
            ...
            m_Log.Info("Fetching sessioninfo 3");
            if (!m_UserSessionService.TryGetSecureValue(secureSessionID, out info))
            {
                return false;
            }
            m_Log.Info("Testing for value 1");
            ...
            m_Log.Info("Fetching sessioninfo 4");
            List<UserSessionInfo> list = m_UserSessionService[ugui];
            if (list.Count != 1)
            {
                return false;
            }
            ...
            m_Log.Info("Testing for expiry of value 3");
            if(info.ContainsKey("testassoc3", "testvarname3"))
            {
                return false;
            }
            ...
            m_Log.Info("Fetching sessioninfo 4");
            list = m_UserSessionService[ugui];
            if (list.Count != 1)
            {
                return false;
            }
