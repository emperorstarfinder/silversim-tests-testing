...
{
    public class PhysicsConvexMeshStorageTests : CommonSimDataTest
    {
        ...
        public override bool Run()
        {
            ...
            var meshid = new UUID("11223344-1122-1122-1122-112233445566");
            PhysicsConvexShape test;

            m_Log.Info("Testing non-existence 1");
            if(m_PhysicsStorage.ContainsKey(meshid, PrimitivePhysicsShapeType.Convex))
            {
                return false;
            }
            m_Log.Info("Testing non-existence 2");
            if(m_PhysicsStorage.TryGetValue(meshid, PrimitivePhysicsShapeType.Convex, out test))
            {
                return false;
            }
            ...
            try
            {
                shape = m_PhysicsStorage[meshid, PrimitivePhysicsShapeType.Convex];
                return false;
            }
            ...
            if (m_PhysicsStorage.Remove(meshid, PrimitivePhysicsShapeType.Convex))
            {
                return false;
            }

            m_Log.Info("Create physics convex shape");
            m_PhysicsStorage[meshid, PrimitivePhysicsShapeType.Convex] = shape;
