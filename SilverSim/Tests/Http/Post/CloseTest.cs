    ...
    public class CloseTest : ITest
    {
        ...
        public bool Run()
        {
            ...
            for (int connidx = 0; connidx++ < NumberConnections;)
            {
                ...
                try
                {
                    res = HttpClient.DoRequest("POST", m_HttpServer.ServerURI + "test", null, "text/plain", connidx.ToString(), false, 60000, HttpClient.ConnectionReuseMode.SingleRequest, headers);
                }
                catch (Exception e)
                {
