    ...
    public class CompressedKeepAliveTest : ITest
    {
        ...
        public bool Run()
        {
            ...
            for (int connidx = 0; connidx++ < NumberConnections;)
            {
                ...
                try
                {
                    res = HttpClient.DoRequest("POST", m_HttpServer.ServerURI + "test", null, "text/plain", connidx.ToString(), true, 60000, connidx == NumberConnections ? HttpClient.ConnectionReuseMode.Close : HttpClient.ConnectionReuseMode.Keepalive, headers);
                }
                catch (Exception e)
                {
