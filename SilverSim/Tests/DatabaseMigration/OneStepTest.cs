...
{
    public sealed class OneStepTest : ITest
    {
        ...
        public bool Run()
        {
            foreach(IDBServiceInterface service in m_MigratableServices)
            {
                ...
                m_MigratorTestControl.StopAtMigrationRevision = 1;
                do
                {
                    m_Log.Info($"Test running to rev {m_MigratorTestControl.StopAtMigrationRevision}");
                    service.ProcessMigrations();
                    m_MigratorTestControl.DeleteTablesBefore = false;
                } while (++m_MigratorTestControl.StopAtMigrationRevision < m_MigratorTestControl.MaxAvailableMigrationRevision);
            }
